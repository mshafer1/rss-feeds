...
def main(
    feeds: typing.Tuple[str, ...],
    output_json: bool,
    out_dir: pathlib.Path,
    cache_dir: pathlib.Path,
    items_path: str,
    item_id_key: typing.Optional[str],
):
    cache_dir.mkdir(parents=True, exist_ok=True)
    out_dir.mkdir(parents=True, exist_ok=True)

    for feed in feeds:
        data = rss_retriever.fetch_feed(
            uri=feed,
            return_json=output_json,
            cache_dir=cache_dir,
        ...
        out_file_name = (
            re.sub(r"[^\dA-Za-z\.\-]", "_", feed)
            + "."
            + hashlib.md5(feed.encode("ascii"), usedforsecurity=False).hexdigest()[:8]
        )
